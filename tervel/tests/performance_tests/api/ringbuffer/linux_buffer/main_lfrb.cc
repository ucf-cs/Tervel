...
/*
 * ------------------------------------------------------------------------
 * tervel wait-free fixed size multi-producer multi-consumer queue
 * ------------------------------------------------------------------------
 */
...
public:
  ...
  pop()
  {
    T *x;
    WrapperType *value;
    while (q_->dequeue(value) == false)
      asm volatile("rep; nop" ::: "memory");
    x = value->value();
    return x;
  }

  const int size_ = QUEUE_SIZE;
 ...
 * ------------------------------------------------------------------------
 */
static const auto N = QUEUE_SIZE * 16;
static const auto CONSUMERS = 32;
static const auto PRODUCERS = 32;
...
struct Producer : public Worker<Q> {
  ...
  void operator()()
  {
    ...
    #ifdef USE_Tervel
    tervel::ThreadContext* thread_context __attribute__((unused));
    thread_context = new tervel::ThreadContext(Worker<Q>::q_->tervel_obj);
    #endif
...
#ifdef USE_Tervel
  TervelQueue<q_type> t_q(PRODUCERS+CONSUMERS);
  run_test<TervelQueue<q_type>>(std::move(t_q));
#endif
  return 0;
}