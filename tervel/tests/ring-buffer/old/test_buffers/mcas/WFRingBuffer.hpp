...
//Shared Memory Pools
std::atomic<RingBuffer::Helper*>	*gl_Helper;

std::atomic<RingBuffer::DWCAS*>	*gl_DWCAS;

std::atomic<RingBuffer::DelayedOp*>	*gl_DelayedOp;
...
//Wait-Free Functions
__thread int helpDelay;
__thread int checkID;
std::atomic<long> *delayedOps;
...
void RingBuffer::helpDelayed(){
	if(threadID==0){
		assert(false); //Should call the bellow function.
		//INIT_THREAD_ID();
	}
...
//#endif
	...
	if(p_lpool_count >= (rb_nThreads+2) ){//If thread pool has too many elements put it in the global pool
		...
		for(count=count; count> (rb_nThreads+2); count--){
		...
		p_lpool_count++;
	#else
		#warning memory pool disabled in wf mcas ring buffer
	#endif
...
static T * getFromMemoryPool(int pThreadID, T* &p_lpool, int &p_lpool_count, std::atomic<T*> * &p_glpool){
	...
	if(res != NULL){//If an unwatched element was found
		...
		int i;
		for(i=0; i<rb_nThreads; i++){ //Iterate each threads
...
void Init_RingBuffer_Memory_Management(int _rb_nThreads){
	rb_nThreads=_rb_nThreads;
	gl_Helper		= new std::atomic<RingBuffer::Helper*>	[_rb_nThreads] ();

	gl_DWCAS		= new std::atomic<RingBuffer::DWCAS*>	[_rb_nThreads] ();

	gl_DelayedOp = new std::atomic<RingBuffer::DelayedOp*>	[_rb_nThreads] ();


	delayedOps	= new std::atomic<long>					[_rb_nThreads] ();
}
