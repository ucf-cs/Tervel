...
public:
	LockFreeQueue(size_t n_producers, size_t n_consumers, unsigned long capacity)
	...
	void push(T *ptr)
	{
		 ...
		 */
		while (__builtin_expect(thr_pos().head >= last_tail_ + capacity_, 0))
		{
			auto min = tail_;
			...
			if (thr_pos().head < last_tail_ + capacity_)
				break;
			_mm_pause();
		}

		ptr_array_[thr_pos().head & mask_] = ptr;
