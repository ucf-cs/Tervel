...
DEFINE_int64(buffer_length, 256, "The size of the buffer to test on.");
DEFINE_int32(operation_type, 0, "The type of test to execute"
    "(0: Both enqueue and dequeue"
    ", 1: Only Enqueue"
    ", 2: Only Dequeue"
    ", 3: Heavy Enqueue (75%enq-25%deq)"
    ", 4: Heavy Dequeue (25%enq-75%deq))";

enum class TestType : size_t {ENQUEUEDEQUEUE = 0, ENQUEUE = 1, DEQUEUE = 2, HEAVYENQUEUE = 3, HEAVYDEQUEUE=4};
...
void run_enqueue_heavy(int thread_id, TestObject * test_data);
void run_dequeue_heavy(int thread_id, TestObject * test_data);
...
void run(int thread_id, tervel::Tervel* tervel_obj, TestObject * test_data) {
  ...
  switch (test_data->operation_type_) {
    ...
    case TestType::ENQUEUEHEAVY:
      run_enqueue_heavy(thread_id, test_data);
      break;

    case TestType::DEQUEUEHEAVY:
      run_dequeue_heavy(thread_id, test_data);
      break;
