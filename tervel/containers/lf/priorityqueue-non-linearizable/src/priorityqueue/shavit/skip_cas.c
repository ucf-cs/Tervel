...
setval_t set_remove(set_t *l, setkey_t k)
{
    ...
    /*need strong search when testing deletemin alone
     * weak search won't remove deleted nodes
     * which is usually done in set_update*/
    x = weak_search_predecessors(l, k, preds, NULL);
    /*x = strong_search_predecessors(l, k, preds, NULL);*/
    if ( x->k > k ) goto out;
...
setkey_t set_deletemin(set_t *l)
{
    sh_node_pt x;
    setkey_t  x_k;
    setval_t  v = NULL, new_v;
    ...
    x = &l->head;
    READ_FIELD(x, x->next[0]);
    x = get_unmarked_ref(x);
    x_k = x->k - 2;

retry_mark:
    while(x != l->tail)
    {
        if(set_remove(l, x_k) != NULL)
        {
            break; 
        }
        else
