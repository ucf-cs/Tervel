...
ctr_node newNode(unsigned long key){
    ctr_node new = (ctr_node) malloc(sizeof(struct ctr_node_t));
    if( new==NULL){
...
ctr_node ctr_init(){
    ctr_node root = newNode(infinity);
...
unsigned long ctr_contains(ctr_node root, unsigned long key ){
    urcu_read_lock();
    ctr_node curr = root->child[0];
    unsigned long ckey = curr->key ;
...
bool validate(ctr_node prev,int tag ,ctr_node curr, int direction){
    bool result;     
    if (curr==NULL){
...
bool ctr_delete(ctr_node root, unsigned long key){
    while(true){
        ...
        ctr_node prevSucc = curr;
        ctr_node succ = curr->child[1]; 

        ctr_node next = succ->child[0];
        while ( next!= NULL){
            prevSucc = succ;
            succ = next;
            next = next->child[0];
